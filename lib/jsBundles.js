...
function jsBundles(app, options, cacheforeverMiddleware, i18nOptions) {
        ...
        async.eachSeries(bundles, function (bundleConfiguration, cb) {
            ...
            if (delayBuildSelector) {
                if (_.isString(delayBuildSelector)) {
                    bundleConfiguration.delayBuild = _.includes(bundlePath, delayBuildSelector);
    ...
    function createBundle(bundleConfiguration, cb) {
        ...
        function buildUsingFactorBundle(bundle, cb) {
            ...
            const {name, factorBundleNames} = bundleConfiguration;
            logger.info("Building factored bundle %s (with %s)...", name, factorBundleNames);
            const outputs = _.map(factorBundleNames, factorBundleName => factoredStream(factorBundleName));
            bundle.plugin(factor, {outputs: outputs});
            bundle.bundle().pipe(factoredStream(name, function (err) {
                const durationInMs = Date.now() - startDate;
                logger.info({timeInMs: durationInMs}, 'Built factored bundle %s (with %s) in %ds',
                    name, factorBundleNames, durationInMs / 1000);
                bundle.building = false;
                cb(err);
            }));
        }
        ...
        function compressJs(src, sourceMapJson, name) {
            let result = {code: src};
            try {
                const minifyResult = Terser.minify(src, _.extend({
                    sourceMap: {
                        content: sourceMapJson,
                        url: name + ".map",
                    },
                }, options.terser, bundleConfiguration.terser));
